...
// *****************    debugging comment    *****************
			...
			else
			{
				...
				//DETERMINE FILE ENTROPY
				//cout << filesystem[i][j].filepath.c_str() << endl;

				
				fileptr = fopen(filesystem[i][j].filepath.c_str(), "r");
				if(fileptr == NULL)
				{
				...
				else if((entropy_value >= 5.0) && (entropy_value < 7.0))
				{
					compressed_size_of_data += entropy_file_size;
					...
					pInfile = fopen(filesystem[i][j].filepath.c_str(), "rb");
					if(!pInfile)
					{
					...
					fclose(pInfile);
					final_compressed_size_of_data += (long long)stream.total_out;
				...
				{
					continue_dedup = true;
					deduped_size_of_data += entropy_file_size;
				}
...
int main()
{
	...
	recursive_search("/home/kwoelfe/dedup/test_programs/dedup_partial_compression_partial/test_files", filesystem, num_files);
...
// *****************    debugging comment    *****************
	...
	cout << "ALL FILES BETWEEN ENTROPY 5 AND 7 ARE SELECTED FOR COMPRESSION." << endl;
	cout << "ALL FILES WITH ENTROPY GREATER THAN 7 ARE IGNORED." << endl << endl;
