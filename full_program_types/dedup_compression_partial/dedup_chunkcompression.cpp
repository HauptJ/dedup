...
long long deduped_and_compressed_size_of_data = 0;
...
long long compress_file(const char* filename)
{
	//////////TIMER START//////////////
	temptime1 = rdtsc();
	///////////////////////////////////
...
long long compress_chunk(const char* filename)
{
	...
	if(!pInfile)
	{
		cout << "Error opening file for chunk compression" << endl;
		exit(1);
	}
	...
	infile_size = (uint)file_loc;
	pOutfile = fopen("compression_test_chunk", "wb");
	if(!pOutfile)
	{
		cout << "Error opening compression output file for chunk." << endl;
		perror("Error: ");
		exit(1);
	}
	...
	for( ; ; )
	{
		...
		if(!stream.avail_in)
		{
			...
			if (fread(s_inbuf, 1, n, pInfile) != n)
			{
				cout << "Failed reading from file for chunk compression." << endl;
				exit(1);
			}
		...
		if ((status == Z_STREAM_END) || (!stream.avail_out))
		{
			...
			//////////TIMER END////////////////
			temptime2 = rdtsc();
			cycles_chunk += temptime2 - temptime1;
			temptime1 = 0;
			temptime2 = 0;
			///////////////////////////////////
			if (fwrite(s_outbuf, 1, n, pOutfile) != n)
			{
				cout << "Error writing chunk compression to file." << endl;
				exit(1);
			}
			//////////TIMER START//////////////
			temptime1 = rdtsc();
...
float get_entropy_file(const char* filename)
{
...
float get_entropy_chunk(const char* filename)
{
	FILE * fileptr = fopen(filename, "r");
	if(fileptr == NULL)
	{
...
// *****************    debugging comment    *****************
			...
			else
			{
				...
				float entropy_value = get_entropy_file(filesystem[i][j].filepath.c_str());
				...
				if((entropy_value >= 5.0) && (entropy_value < 7.0))
				{
					...
					//COMPRESS FILE HERE
					long long compression_sizing = compress_file(filesystem[i][j].filepath.c_str());
				...
				for(int k = 0; k < numchunks; k++)
				{
					...
					//WRITE THE BUFFER TO DISC, SO IT CAN HAVE ITS ENTROPY
					//MEASURED AND COMPRESSED. NOT PART OF THE NORMAL
					//DEDUP PROCEDURE, SO DON'T MEASURE CYCLES

					FILE *chunkfile = fopen("chunkfile", "wb");
					if(!chunkfile)
					{
						cout << "Error opening compression chunk output file." << endl;
						exit(1);
					}
					if (fwrite(file_buffer, 1, chunksize_inbytes, chunkfile) != chunksize_inbytes)
					{
						cout << "Error writing compression chunk file to disc." << endl;
						exit(1);
					}
					//rewind(chunkfile);
					//cout << filesystem[i][j].filepath.c_str() << " : " << chunksize_inbytes << endl;
									

					//Now that the chunk is written to disc, measure it's entropy
					float chunk_entropy_value = get_entropy_chunk("chunkfile");

					//Compress the chunk if its worth it, based on entropy
					if(chunk_entropy_value < 7.0)
					{
						long long compressed_chunk_size = compress_chunk("chunkfile");
						deduped_and_compressed_size_of_data += compressed_chunk_size;
					}
					else
					{
						long long uncompressed_chunk_size = chunksize_inbytes;
						deduped_and_compressed_size_of_data += uncompressed_chunk_size;
					}
					fclose(chunkfile);

					
					//END CHUNK COMPRESSION
...
// *****************    debugging comment    *****************
	...
	cout << "Dedup Portion Size (after chunks compressed), " << deduped_and_compressed_size_of_data << endl;
	...
	cout << "Cycles Spent Compressing Dedup Chunks, " << cycles_chunk << endl;
	cout << endl;
	return 0;
}
