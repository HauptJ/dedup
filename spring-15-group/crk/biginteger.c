...
int integer_poly_hasdegree(integer a, int deg){
  if (deg/8 < BYTES){
    if ((a.byte[BYTES-1-deg/8] >> (deg%8)) & 0x1)
      return 1;
  }
...
integer integer_poly_mod(integer a, integer b){
  integer result = integer_initint(a);
  int da = integer_poly_degree(a);
  int db = integer_poly_degree(b);
  
  int i;
  for (i = da - db; i >= 0; i--){
    if (integer_poly_hasdegree(result, i+db)){
      integer shifted = integer_initint(integer_shiftl_times(b, i));
      result = integer_xor(result, shifted); 
    }
  }
...
integer integer_poly_gcd(integer a, integer b){
  integer zero = integer_init();
  integer ret = integer_initint(a);
  while (!integer_eq(b, zero)){
    integer temp = integer_initint(b);
    b = integer_poly_mod(ret, b);
    ret = temp;
  }
  return ret;
}
...
integer integer_poly_modPow(integer base, integer exponent, integer modulus){  
  integer result = integer_initval(1);
  integer zero = integer_init();
  integer b = integer_initint(base);
  while(!integer_eq(exponent, zero)){
    if (exponent.byte[BYTES-1] & 0x1){
      result = integer_poly_mod(integer_poly_mult(result, b), modulus);
    }
    exponent = integer_shiftr(exponent);
    b = integer_poly_mod(integer_poly_mult(b, b), modulus);
  }
  return result;
}
...
void integer_gen_test_print(){
  ...
  // 2bac = 2 3 5 7 8 9 11 13
  // 13 7 6 11 5 4 9 3 2 8 2 1 7 1 0 
  
  // 0x57 * 0x83 = 0x2B79
  //  integer_printhex(integer_poly_mult(integer_initval(0x57),
  //				     integer_initval(0x83)));	
  printf("poly_mult %s\n",
	 (integer_eq(integer_poly_mult(integer_initval(0x57),
				       integer_initval(0x83)),
		     integer_initval(0x2B79)))?
	 "ok":"failed");
  
  // 6 4 1 0 * 7 6 3 1
  // 13 12 9 7 11 10 7 5 8 7 4 2 7 6 3 1
  // 13 12 11 10 9 8 6 5 4 3 2 1 
  // 13 12 11 10 9 8 6 5 4 3 2 1 
  printf("poly_mult %s\n",
	 (integer_eq(integer_poly_mult(integer_initval(0x53),
				       integer_initval(0xCA)),
		     integer_initval(0x3F7E)))?
	 "ok":"failed");
  
  printf("poly_mod %s\n",
	 (integer_eq(integer_poly_mod(integer_initval(0x3F7E),
				      integer_initval(0x11B)),
		     integer_initval(0x1)))?
	 "ok":"failed");
  
  printf("poly_gcd %s\n",
	 (integer_eq(integer_poly_gcd(integer_initval(0x3F7E),
				      integer_initval(0x11B)),
		     integer_initval(0x1)))?
	 "ok":"failed");
  
  //(x^6 + x^4 + x^2 + x + 1) (x^7 + x + 1) modulo (x^8 + x^4 + x^3 + x + 1)
  // equals x^7 + x^6 + 1
  //0x57^0x83 % 0x11B = 0xC1
  printf("poly_modPow %s\n",
	 (integer_eq(integer_poly_modPow(integer_initval(0x57),
					 integer_initval(0x83),
					 integer_initval(0x11B)),
		     integer_initval(0xC1)))?
	 "ok":"failed");
  
  integer_printhex(integer_poly_mult(integer_initval(0x57), integer_initval(0x1)));
  integer_printhex(integer_poly_modPow(integer_initval(0x57),
				       integer_initval(0x83),
				       integer_initval(0x11B)));
