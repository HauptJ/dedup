...
char *toStringHex(bits l){
    char *hexString;
    int i;
    hexString=(char*)malloc(BYTES*2+1); // two hex chars make up 1 byte, add 1 for term char)
    for (i = 0; i < BYTES; i++)
...
char *toStringBin(bits l){
    ...
    char* tempHexString;
    tempHexString=(char*)malloc(BYTES*2+1); // two hex chars make up 1 byte, add 1 for term char)
    tempHexString=toStringHex(l);

    //our main string
    char* binString;
    binString=(char*)malloc((BYTES*2+1)*4); // it takes 4 bits to represent one hex so the sting needs be 4 times as long
    int i = 0;
