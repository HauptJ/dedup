...
char *toStringHex(bits l){
    char *hexString;
    int i;
    hexString=(char*)malloc(l.size*2+1); // two hex chars make up 1 byte, add 1 for term char)
    for (i = 0; i < l.size; i++)
    	sprintf(hexString+2*i, "%02X", l.byte[i]);
    return hexString;
}
...
char *toStringBin(bits l){
    //hex is a nice starting point for binary
    char* tempHexString;
    tempHexString=(char*)malloc(l.size*2+1); // two hex chars make up 1 byte, add 1 for term char)
    tempHexString=toStringHex(l);

    //our main string
    char* binString;
    binString=(char*)malloc((l.size*2+1)*4); // it takes 4 bits to represent one hex so the sting needs be 4 times as long
    int i = 0;
...
/*
char*toStringPoly(bits l){// backwards //space between char* toStringPoly maybe?
    //x^n+x^(n-1)...+X+C
    char* tempBinString;
    tempBinString=(char*)malloc((BYTES*2+1)*4);
    tempBinString=toStringBin(l);

     char* polyString;
    polyString=(char*)malloc((BYTES*2+1)*40); // it takes 4 bits to represent one hex so the sting needs be 4 times as long
    int i = 0;
